...
static void i2s_config(void)
{
	...
	io_conf.pin_bit_mask  = 1ULL << GPIO_NUM_23; // PTT0
	io_conf.pin_bit_mask |= 1ULL << GPIO_NUM_27; // STA0
	io_conf.pin_bit_mask |= 1ULL << GPIO_NUM_22; // PTT1
	io_conf.pin_bit_mask |= 1ULL << GPIO_NUM_21; // STA1
	io_conf.pull_down_en = 0;
	io_conf.pull_up_en = 0;
	gpio_config(&io_conf);
	gpio_set_level(GPIO_NUM_23, 0); // off
	gpio_set_level(GPIO_NUM_27, 0); // off
	gpio_set_level(GPIO_NUM_22, 0); // off
	gpio_set_level(GPIO_NUM_21, 0); // off

	io_conf.mode = GPIO_MODE_OUTPUT_OD; // open drain
	io_conf.pin_bit_mask  = 1ULL << GPIO_NUM_14; // DCD0
	io_conf.pin_bit_mask |= 1ULL << GPIO_NUM_19; // DCD1
	gpio_config(&io_conf);
	gpio_set_level(GPIO_NUM_14, 1); // off
	gpio_set_level(GPIO_NUM_19, 1); // off


#if 0
    static const uint8_t led[6] = {
	GPIO_NUM_23, GPIO_NUM_14, GPIO_NUM_27,
	GPIO_NUM_22, GPIO_NUM_19, GPIO_NUM_21,
    };
    for (int i = 0;; i++) {
	gpio_set_level(led[i % 6], (i % 3 != 1) ? 1 : 0);
	vTaskDelay(1000 / portTICK_PERIOD_MS);
	gpio_set_level(led[i % 6], (i % 3 != 1) ? 0 : 1);
    }
#endif
...
static uint16_t i2s_read_buff[I2S_DMA_BUF_LEN * 2]; // *2 for stereo
static uint16_t i2s_write_buff[I2S_DMA_BUF_LEN * 2]; // *2 for stereo
static const int i2s_buff_len = sizeof(i2s_read_buff);
...
static void i2s_read_task(void *p)
{
    size_t bytes_read;
    static size_t bytes_read_prev = 0;
    uint32_t t0, t1, prev_time = 0;

    while (1) {

    //gpio_set_level(I2S_GPIO_PIN, 0);
    //read data from I2S bus, in this case, from ADC.
    ESP_ERROR_CHECK(i2s_read(I2S_NUM, (void*) i2s_read_buff, i2s_buff_len, &bytes_read, portMAX_DELAY));
	...
	if (xRingbufferSend(rxd_ringbuf, i2s_read_buff, samples , 0) != pdTRUE) {
...
#define PTT1_GPIO GPIO_NUM_22
    ...
    // PTT off
    if ((ptt0 > 0) && (--ptt0 == 0)) {
	gpio_set_level(PTT0_GPIO, 0);
	xEventGroupSetBits(i2s_event_ptt, 1 << 0);
    }
    if ((ptt1 > 0) && (--ptt1 == 0)) {
	gpio_set_level(PTT1_GPIO, 0);
...
#define I2S_BUF_ONE_BIT_LEN (I2S_DMA_BUF_LEN / I2S_DMA_BUF_BITS)
    ...
    memcpy(&i2s_write_buff[i * I2S_BUF_ONE_BIT_LEN * CH_N], sin_table[p0][p1][f0][f1], I2S_BUF_ONE_BIT_LEN * CH_N * sizeof(uint16_t));
    ...
    ESP_ERROR_CHECK(i2s_write(I2S_NUM, (void*) i2s_write_buff, i2s_buff_len, &bytes_written, portMAX_DELAY));
    ...
    if (prev_time != 0) {
	t1 = t0 - prev_time;
	if (t1 > i2s_period_max_tx) {
	    i2s_period_max_tx = t1;
	}
    }
...
#define I2S_PRIORITY (configMAX_PRIORITIES - 1)

    res = xTaskCreate(i2s_read_task, "i2s_read_task", 1024 * 2, ringbuf, I2S_PRIORITY, NULL);
    if (res != pdTRUE) {
	ESP_LOGE(TAG, "xTaskCreate fail");
	vTaskDelete(NULL);
    }
    res = xTaskCreate(i2s_write_task, "i2s_write_task", 1024 * 2, ringbuf, I2S_PRIORITY, NULL);
    if (res != pdTRUE) {
	ESP_LOGE(TAG, "xTaskCreate fail");
	vTaskDelete(NULL);
    }
